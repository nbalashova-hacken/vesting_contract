...
describe("MultiVesting", function () {
  ...
  describe("currrentTimestamp", function () {
    it("Should return the current block timestamp", async function () {
      const { multiVesting } = await loadFixture(deployFixture);

      const currentTimestamp = await multiVesting.currrentTimestamp();
  ...
  describe("getAddressBalance", function () {
    it("Should return the balance of the given address", async function () {
      const { multiVesting, owner } = await loadFixture(deployFixture);

      const addressBalance = await multiVesting.getAddressBalance(owner.address);
      const actualBalance = await hre.ethers.provider.getBalance(owner.address);

      expect(addressBalance).to.equal(actualBalance);
    });
  });
});
